...
class EvalAgentTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        def gradeThreshold(value, minimum, thresholds, weight, name):
            points = 0
            ...
            if passed:
                for t in thresholds:
                    if value >= t:
                        points += weight
            return (passed, points, value, minimum, thresholds, weight, name)

        results = [gradeThreshold(averageScore, self.scoreMinimum, self.scoreThresholds, 2, "average score"),
                   gradeThreshold(nonTimeouts, self.nonTimeoutMinimum,
                                  self.nonTimeoutThresholds, 1, "games not timed out"),
                   gradeThreshold(wins, self.winsMinimum, self.winsThresholds, 1, "wins")]

        totalPoints = 0
        for passed, points, value, minimum, thresholds, weight, name in results:
            ...
            if not passed:
                ...
                self.addMessage("%s %s (%s of %s points)" %
                                (value, name, points, len(thresholds) * weight))
